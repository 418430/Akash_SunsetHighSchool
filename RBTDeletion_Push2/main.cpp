//Akash Pai
//Friday, 5/12/2023
//This project is Red-Black-Tree Deletion. This is push 2. Numbers are read in from file only, this time, not from command line, and are sorted in a binary tree that happens to be balanced.
...
//In this push, I have added two functions that find siblings and a given node's successor. Additionally, I have written function prototype for delete, as well as the function prototype for checking various cases.
#include <iostream>
#include <fstream>
#include <cstring>
...
void remove(Node* root, Node* v); //receieved function prototype from N. Nayak & Uno
void checkCases(..., ..., ...)
...
Node* sibling(Node* current) { //sibling function (retrieves)
    if (current == current->getParent()->getLeft()) { //case 1
...
Node* findHeir(Node* current) {//finds successor essentially
    current = current->getRight();
    while(current->getLeft() != NULL) {//iteration
        current = current->getLeft();
    }

    return current; //returns to whatever function needs it
}

void remove(Node* root, Node* v) {
	//important to check for cases, apprently having two different black nodes is a big one
}

void checkCases(Node *, Node *) {
	//TO:DO:
	//check portfolio
	//Robert Kiyosaki Webinar
	//Finish Rich Dad Poor Dad
	//Lit Narrative Submission
	//Submit Publication
	//Sleep by 11
}