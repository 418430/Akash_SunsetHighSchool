//Akash Pai
//Thursday, 4/27/23
//This project is Red-Black-Tree Insertion (RBT P1). Numbers are read in from file only, this time, not from command line, and are sorted in a binary tree that happens to be balanced.
...
int main() {
    ...
    while (stillPlaying == true) {
        cout << "Enter the diff moves. Input by File (up to 50 nums) (Enter: FILE), display (Enter: DISPLAY), remove (Enter: REMOVE), search (Enter: SEARCH), or quit(Enter: QUIT)" << endl;
        char input[100];
        cin >> input;
...
bool search(Node* root, int value) { //search function - finally sum easy to implement do this tmrw
    Node* current = root;
    while (current != NULL) {
    ...
    return false; //if we reach the final null node (current == NULL) without returning true, we know we have traversed the whole trree without finding the value. in this case, then, yes, return false as value is not there.
}
